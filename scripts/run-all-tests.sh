...
# Find all files in mdremotifier that end in _test.py
find mdremotifier -name "*_test.py" -print0 | while IFS= read -r -d '' TEST_FILE; do
...
find mdremotifier -name "*_test.sh" -print0 | while IFS= read -r -d '' TEST_FILE; do
  echo -e "${YELLOW}Running ${TEST_FILE}${NC}"
  bash "${TEST_FILE}"
  echo -e "${GREEN}${TEST_FILE} ran successfully${NC}"
done
